...
async function getCurrentLocation(): Promise<{ latitude: number; longitude: number } | null> {
  // Geolocation is not typically available in a serverless function context.
  // This function is kept for completeness if location-aware features were to be added on client,
  // but for serverless context, it will always resolve to null.
...
export default async function handler(req: any, res: any) {
  ...
  const { movieName, selectedVoice, selectedContentLanguage } = req.body;
  ...
  if (!apiKey) {
    ...
    return res.status(500).json({ error: "Authentication failed. Server API Key is not configured." });
  }
  ...
  try {
    ...
    const speechResponse = await ai.models.generateContent({
      ...
      config: {
        ...
        speechConfig: {
          voiceConfig: {
            prebuiltVoiceConfig: { voiceName: selectedVoice }, // Use the selected voiceName
          },
        },
      },
    });
...
If you enjoyed this video, please like, share it with your friends, and subscribe to Pujiverse Cinema. Hit the bell icon for more updates!
    ...
    if (selectedContentLanguage === 'telugu-mix') youtubeHashtags.push('#TeluguEnglishMix');
    if (selectedContentLanguage === 'hindi-mix') youtubeHashtags.push('#HindiEnglishMix');
    ...
    if (selectedContentLanguage === 'telugu-mix') youtubeTags.push('Telugu English mix', 'Telugu film review');
    if (selectedContentLanguage === 'hindi-mix') youtubeTags.push('Hindi English mix', 'Hindi film review');
    ...
    if (error.message && (error.message.includes("Authentication failed") || error.message.includes("Requested entity was not found."))) {
      ...
      throw new Error("Authentication failed. Please ensure a valid API key is selected on your Vercel project.");
    }
    // For generic 500 errors or other unknown errors
    throw new Error(`Failed to generate movie voice-over: ${error.message || 'Unknown error'}. Please try again, or check your Vercel project's API key configuration.`);
  }
}